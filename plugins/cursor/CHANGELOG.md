...
- onboarding: new Setup skill that connects a warehouse to Monte Carlo over the REST API v2 MCP tools — `list_deployments` first, then reuse the hosted cloud node or provision a deployment for a collection agent or a customer-owned data store (never one with nothing behind it), credentials by reference (no secret through the chat; Snowflake key pairs and generic-agent tokens become a CLI/Terraform step), warehouse, connection, "validate in the UI", and a summary of created ids on every run. Three output modes: act through the tools, emit Terraform (`terraform-provider-montecarlo` + `mcd-agent` modules), or emit an SDK/CLI script. Invoked by describing the task; the slash command and trigger evals ship in the Claude Code and Cortex Code plugins (YET-2897)
