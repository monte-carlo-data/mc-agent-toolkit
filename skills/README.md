...
| **[Onboarding](onboarding/)** | Connects a warehouse to Monte Carlo through the REST API v2 tools — deployment choice (cloud node, collection agent, data store), credential references, warehouse and connection, with Terraform and SDK/CLI script output modes. Ends every run with the created ids. |
