...
- **If the customer is on the serverless template (custom `span_processor`), did they pass `MCD_DEFAULT_*` headers explicitly to `OTLPSpanExporter`?** `mc.setup()` only auto-injects auth headers when it builds the default exporter; with a custom `span_processor` the customer constructs the exporter, so the headers are their responsibility. Either `OTLPSpanExporter(endpoint=..., headers={"x-mcd-id": ..., "x-mcd-token": ...})` or `OTEL_EXPORTER_OTLP_HEADERS` set in the env so `OTLPSpanExporter` reads it. Without one of those, the env vars exist but never reach the wire — symptom looks like missing credentials but the actual bug is the exporter is unauthenticated. See the upstream README's [Auth headers note](https://github.com/monte-carlo-data/montecarlo-opentelemetry/blob/main/README.md#serverless-and-other-suspendable-runtimes).
