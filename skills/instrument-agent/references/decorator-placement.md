...
### `@mc.trace_with_workflow(span_name, workflow_name)` — orchestration functions

A function counts as **orchestration** when:

- It coordinates a sequence of operations across multiple LLM calls or tool calls.
- It's the entry point of a logical agent flow (e.g., a LangGraph graph node that calls multiple sub-functions).
...
1. **Identify the entry point.** What's the top-level function the user calls to run the agent? That's a workflow candidate.
...
3. **Identify intermediate orchestration nodes.** Multi-step functions between the entry and the LLM call sites — workflow candidates.
...
**IMPORTANT — Don't over-decorate.** Aim for 1 workflow per logical agent flow + 1 task per LLM call. Decorating every helper function adds span noise without semantic value.
