...
**Workflow 5 — Change validation queries:** After you've made a change and are ready to test it, say something like "generate validation queries" or "validate this change" — or run `/mc-validate`. Claude generates 3–5 targeted SQL queries based on the Workflow 4 findings and the diff — null checks, before/after row counts, distribution checks — using the exact column names, filter logic, and business rules from your change. Queries are saved to `validation/<table_name>_<timestamp>.sql` with inline comments explaining what a passing result looks like for each check. Always ends by offering `/mc-validate run` (Workflows 6 + 7 below) as the next step. Skipped entirely for YAML/docs-only diffs, since there's no SQL behavior to validate.

**Workflow 6 — Sandbox build:** Triggered by `/mc-validate run`. Parses your `profiles.yml`, classifies the resolved database (personal / dev / shared-dev / prod / unknown), detects hard-coded `database:` overrides in the model's `{{ config() }}`, and offers to run `dbt build --select <model>` into your dev database so the validation queries have real data to read from. Hard-stops if the resolved database is a shared prod database, and skips automatically for YAML/docs-only diffs.

**Workflow 7 — Execute validation queries:** Triggered by `/mc-validate run`, immediately after Workflow 6. Substitutes `<YOUR_DEV_DATABASE>` in the generated SQL with a user-confirmed value, surfaces every database the queries will touch (including literal prod references for parity checks), enforces read-only before execution, runs each query through the Snowflake MCP, and reports per-query ✅/⚠️/🔴 verdicts plus a consolidated summary grounded in each query's "What to look for" comment.
